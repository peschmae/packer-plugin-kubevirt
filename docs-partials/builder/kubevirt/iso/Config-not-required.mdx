...
- `storage_class_name` (string) - StorageClassName is the name of the storage class to use for the root disk.
...
- `memory` (string) - Memory is the amount of memory to allocate to the VM (e.g. "4Gi").
  Required when instance_type is not set. Cannot be used together with instance_type.

- `cpu` (uint32) - CPU is the number of CPU cores to allocate to the VM.
